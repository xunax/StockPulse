...
with c2:
    if st.button("☰", key="btn_hamburger"):
        ...
        st.markdown("<script>setTimeout(function(){var b=document.querySelector('[data-testid=collapsedControl]');if(b)b.click();},50);</script>", unsafe_allow_html=True)
with c3:
...
if st.session_state.get("show_user"):
    ...
    if st.sidebar.button("🚪 登出", key="btn_logout", use_container_width=True):
        st.session_state["logged_in"] = False
        st.session_state["username"] = ""
        st.rerun()
